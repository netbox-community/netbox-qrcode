...
* `logo`: 
    ...
    ℹ️ To keep the traffic and performance high, the image should be as small as necessary. An image with 1920x1200px and 30MB is nonsensical if it is to be 1.00x3.00cm in size at the end. The larger the image, the longer the Base64 text. If possible, the logo is completely black and not gray or colored, which makes it even smaller. (Sufficient for thermal transfer printers).
    ...
    ### Version 1: Link to the image (Recommended)

    The media folder to provide the plugin with images is located here:
    ...
    ℹ️ You can specify a link to the image file in Netbox or an external http:// or https:// link as long as the client can reach this image.
 
    ### Version 2: Embedded Base64 Image (not recommended due to complexity)
